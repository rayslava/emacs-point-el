...
- Для моментального выбора id/имени/тэга для вставки в поле редактирования `M-g i`/`M-g u`/`M-g t`. С префиксным аргументом (например `M-1 M-g i`) не вставляет, а просто активирует выбранную сущность. Требует [avy](https://github.com/abo-abo/avy);
